...
@interface ARRadar ()
@property (strong, nonatomic) NSArray *radarData;
@end
...
{
    if (_theSpots.count < 1) return;
    
    CGContextRef contextRef = UIGraphicsGetCurrentContext();
    
    CGContextSetRGBFillColor(contextRef, 0.2, 1, 0.2, 0.8);
    
    int max = [[[_theSpots allValues] valueForKeyPath:@"@max.intValue"] intValue];
    int min = [[[_theSpots allValues] valueForKeyPath:@"@min.intValue"] intValue];
    
    int x = 0;
    int y = 0;
    
    for (NSNumber *angle in _theSpots.allKeys) {
        ...
        if ([[_theSpots objectForKey:angle] intValue] != min) {
            distModifier = 1-(([[_theSpots objectForKey:angle] floatValue]-min)/(max-min));
        }
...
- (void)showDots {
    ...
    for (NSDictionary* spot in _radarData) {
        ...
        [_theSpots setObject:[NSNumber numberWithInt:dist]
...
- (id)initWithFrame:(CGRect)frame withSpots:(NSArray*)spots
{
    ...
    if (self) {
        ...
        _theSpots = [NSMutableDictionary dictionary];
...
- (NSString *)description {
    return [NSString stringWithFormat: @"ARRadar with %lu dots", (unsigned long)_theSpots.count];
}
